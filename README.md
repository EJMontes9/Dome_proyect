...
> **Nota importante:** La imagen Docker oficial `bitnami/moodle` fue migrada a Bitnami Secure Images y ya no esta disponible en Docker Hub. Este proyecto usa `bitnamilegacy/moodle:4.5` como reemplazo compatible.
...
La primera vez, Moodle tarda **entre 1 y 3 minutos** en inicializarse completamente. Puedes monitorear el progreso con:
...
> **Nota:** Los datos se persisten en volumenes Docker (`docker_mariadb_data`, `docker_moodle_data`, `docker_moodledata`), asi que al volver a levantar los contenedores no perderas la configuracion.
...
> **Nota sobre Google OAuth:** Para obtener el `GOOGLE_CLIENT_ID`, ve a [Google Cloud Console](https://console.cloud.google.com/), crea un proyecto, habilita la API de Google Sign-In y crea credenciales OAuth 2.0 de tipo "ID de cliente de aplicacion web". Agrega `http://localhost:8001` como origen autorizado.
...
> **Nota:** Si el puerto 8001 esta ocupado, puedes usar otro puerto libre con `--port NUMERO`.
...
# 2. Backend (FastAPI)
cd backend; .\venv\Scripts\Activate; uvicorn app.main:app --reload --port 8001
...
- Asegurate de esperar 1-3 minutos en la primera ejecucion
- Verifica que el puerto 8090 no este ocupado por otro servicio: `netstat -ano | findstr :8090` (Windows) o `lsof -i :8090` (Linux/macOS)
...
- En algunos sistemas Linux necesitas instalar `python3-venv`: `sudo apt install python3-venv`
...
- Borra `node_modules` y reinstala: `rm -rf node_modules && npm install` (Linux/macOS) o `Remove-Item -Recurse node_modules; npm install` (Windows)
...
| **LMS** | Moodle 4.5 (Bitnami Legacy Docker) |
